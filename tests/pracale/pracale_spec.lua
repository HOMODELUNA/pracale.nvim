require("plenary.reload").reload_module("pracale", true)
local pracale = require("pracale")
local default = pracale.config
...
describe("tests", function()
  it("works with default values", function()
    pracale.setup()
    assert.are.same(pracale.config, default)
  end)
  ...
  it("works with config overrides", function()
    ...
    pracale.setup({ undercurl = false, underline = false })
    assert.are.same(pracale.config, expected)
  end)
  ...
  it("does not set terminal colors when terminal_colors is false", function()
    clear_term_colors()
    pracale.setup({ terminal_colors = false })
    pracale.load()
  ...
  it("sets terminal colors when terminal_colors is true", function()
    clear_term_colors()
    pracale.setup({ terminal_colors = true })
    pracale.load()

    -- dark bg
    local colors = require("pracale").palette
